...
void Parser::Statement(ASTU &stat)
{
	...
	case 17 /* "ret" */:
	{
		...
		{
			Expr(e_stat);
			if (!sreq(symbol_table::currMethod()->type->elemType, get_obj(e_stat)->type))
			{
				cout << ";El valor q retorna no coincide con la firma. " << *symbol_table::currMethod()->name << endl;
				exit(1);
			}
			stat = return_node(e_stat);
			set_obj(stat, get_obj(e_stat));
		}
		else if (la->kind == 11 /* ";" */)
		{
			stat = return_node();
...
void Parser::VarDeclDef(ASTU &s)
{
	...
	for (int i = 0; i < c_declaraciones; i++)
	{
		...
		cout << ";adr: " << symbol_table::insert(Obj::Var, declaraciones[i]->tmpName, declaraciones[i]->type)->adr << endl;
		;
...
void Parser::Designator(ASTU &d_ast)
{
	...
	else if (la->kind == _t_pointer)
	{
		...
		Factor(d_ast);
		d_ast = (d_ast);
		cout << "; desreferenciar " << *get_obj(d_ast)->name << endl;
		un_pointer(get_obj(d_ast));
		dump(get_obj(d_ast)->type, 2);
	}
	else
		SynErr(50);
	if (StartOf(3))
	{
		while (la->kind == 45 /* "." */)
		{
			...
			Expect(_name);
			LexString(ns);
			*obj = *symbol_table::findField(ns, obj->type);
			cout << "; load " << *obj->name << endl;
		}
	}
	else if (la->kind == 22 /* "[" */)
	{
...
{
	Obj *obj = get_obj(f_node);
	int n = 0;
	f_node = func_call(f_node, ifc);
	Expect(14 /* "(" */);
	if (StartOf(2))
	{
		ASTU s_2;
...
void Parser::FuncDecl(ASTU &f_node)
{
	...
	Tipe = new string("");
	LexString(Tipe);
	*last(fd->type) = *symbol_table::find(Tipe)->type;
	cout << ";" << *Tipe << endl;
	funcion(fd->type);
	dump(fd->type, 1);
...
void Parser::VarSuffix(MetaData *md)
{
	...
	else if (la->kind == 14 /* "(" */)
	{
		...
		Expect(15 /* ")" */);
		funcion(last(md->type));
		md->isF = true;
		;
		cout << " ;function q retorna";
	}
	else
		SynErr(55);
}
...
void Parser::Factor(ASTU &f_node)
{
	...
	{
		Get();
		*obj = *new Obj(0, new string(""), symbol_table::boolType);
		f_node = const_int_node(1);
		set_obj(f_node, obj);
		break;
	}
	case 44 /* "false" */:
...
void Errors::SynErr(int line, int col, int n)
{
	...
	case 0:
		...
		break;
	case 1:
		s = coco_string_create(L"t_pointer expected");
		break;
	case 2:
		s = coco_string_create(L"t_r_pointer expected");
		break;
	case 3:
	...
	case 4:
		...
		break;
	case 5:
		s = coco_string_create(L"charConst expected");
		break;
	case 6:
		s = coco_string_create(L"stringConst expected");
		break;
	case 7:
		s = coco_string_create(L"floatLit expected");
		break;
	case 8:
		s = coco_string_create(L"binaryLit expected");
		break;
	case 9:
	...
	case 12:
		...
		break;
	case 13:
		s = coco_string_create(L"\"print\" expected");
		break;
	case 14:
		s = coco_string_create(L"\"(\" expected");
		break;
	case 15:
		...
		break;
	case 16:
		s = coco_string_create(L"\"sizeof\" expected");
		break;
	case 17:
		s = coco_string_create(L"\"ret\" expected");
		break;
	case 18:
	...
	case 23:
		...
		break;
	case 24:
		s = coco_string_create(L"\"extern\" expected");
		break;
	case 25:
		s = coco_string_create(L"\"fun\" expected");
		break;
	case 26:
		s = coco_string_create(L"\"<<\" expected");
		break;
	...
	case 46:
		...
		break;
	case 47:
		s = coco_string_create(L"invalid Statement");
		break;
	case 48:
		s = coco_string_create(L"invalid Statement");
		break;
	case 49:
		...
		break;
	case 50:
		s = coco_string_create(L"invalid Designator");
		break;
	case 51:
		s = coco_string_create(L"invalid Designator");
		break;
	case 52:
		s = coco_string_create(L"invalid FuncDecl");
		break;
	case 53:
		s = coco_string_create(L"invalid VarDcl");
		break;
	case 54:
		s = coco_string_create(L"invalid VarDirectDcl");
		break;
	case 55:
		s = coco_string_create(L"invalid VarSuffix");
		break;
	case 56:
		s = coco_string_create(L"invalid FuncDcl");
		break;
	case 57:
		s = coco_string_create(L"invalid FuncDirectDcl");
		break;
	case 58:
		s = coco_string_create(L"invalid FuncSuffix");
		break;
	case 59:
